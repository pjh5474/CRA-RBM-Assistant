from typing import List

from fastapi import APIRouter, Depends

from app.schemas.action_item_schema import SiteActionItemsResponse
from app.schemas.study_schema import (
    SiteResponse,
    StudyDetailResponse,
    StudyRiskSiteResponse,
    StudySummaryResponse,
)
from app.services.action_item_service import get_action_items_by_study_id
from app.services.study_service import (
    get_accessible_studies,
    get_all_studies,
    get_risk_sites_by_study_id,
    get_sites_by_study_id,
    get_study_by_id,
)
from app.utils.auth import get_optional_authenticated_user

router = APIRouter(prefix="/api", tags=["studies"])


@router.get("/studies", response_model=List[StudySummaryResponse])
def get_studies():
    return get_all_studies()


@router.get("/studies/{study_id}", response_model=StudyDetailResponse)
def get_study_detail(study_id: str):
    return get_study_by_id(study_id)


@router.get("/studies/{study_id}/sites", response_model=List[SiteResponse])
def get_study_sites(study_id: str):
    return get_sites_by_study_id(study_id)


@router.get(
    "/studies/{study_id}/risk-sites", response_model=List[StudyRiskSiteResponse]
)
def get_study_risk_sites(study_id: str):
    return get_risk_sites_by_study_id(study_id)


@router.get(
    "/studies/{study_id}/action-items", response_model=List[SiteActionItemsResponse]
)
def get_study_action_items(study_id: str):
    return get_action_items_by_study_id(study_id)


@router.get("/accessible/studies", response_model=List[StudySummaryResponse])
def get_accessible_studies_endpoint(
    current_user=Depends(get_optional_authenticated_user),
):
    owner_user_id = current_user["id"] if current_user else None
    return get_accessible_studies(owner_user_id=owner_user_id)
